...
<p><img src="https://raw.githubusercontent.com/rel-start/Notes/picture/picture/symbol-xmind.png" /></p>
...
<hr/>
<h2>2. Symbol.prototype.description</h2>
...
上面代码中，因为点运算符后面总是字符串，所以不会读取mySymbol作为标识名所指代的那个值，导致`a`的属性名实际上是一个字符串，而不是一个 Symbol 值。
...
上面代码中，如果s不放在方括号中，该属性的键名就是字符串`s`，而不是`s`所代表的那个 Symbol 值。
...
<hr/>
<h2>6. Symbol.for()，Symbol.keyFor()</h2>
...
// 类似于
function MyClass(){ }
MyClass.prototype[Symbol.hasInstance] = (foo) => {
  return foo instanceof Array;
}
...
上面代码中，由于定义了`Symbol.species`属性，创建衍生对象时就会使用这个属性返回的函数，作为构造函数。这个例子也说明，定义`Symbol.species`属性要采用get取值器。默认的`Symbol.species`属性等同于下面的写法。
