...
上面代码中，字符串 str 的`Symbol.iterator`方法被修改了，所以扩展运算符（`...`）返回的值变成`了`bye`，而字符串本身还是`hi`。

<hr/>
<h2>5. Iterator 接口与 Generator 函数</h2>
...
第二种解决办法是，在`Symbol.iterator`里面做文章。
...
let es6 = {
  ...
  [Symbol.iterator]() {
    return {
      index: 0,
      next() {
        let keysArr = Object.keys(es6), len = keysArr.length;

        if (this.index < len) {
          return { value: es6[keysArr[this.index++]], done: false };
        }
        return { done: true }
      }
    }
